...
def parse_expense(ctx: Context, node_input) -> Event:
    ...
    # Route based on threshold rule (Python code-based routing)
    route = "auto_approve" if amount < APPROVAL_THRESHOLD else "needs_review"
...
async def human_approval_gate(ctx: Context, node_input: dict | str | None = None):
    """Pauses the workflow for human approval/rejection and reports the AI risk assessment."""
...
root_agent = Workflow(
    ...
    edges=[
        ...
        (security_checkpoint, {"clean": risk_reviewer, "flagged": human_approval_gate}),
        (risk_reviewer, human_approval_gate),
